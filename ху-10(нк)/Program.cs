...
class bookstore : Book
{
    ...
    public bookstore(string? name, string? author, int year, int pagesize, int newprice) : base(name, author, year, pagesize)
    {
        Newprice = newprice;
    }
    public virtual void lowerprice(int value)
    {
        this.value = value;
        if (Year - DateTime.Now.Year == 5)
        {
            value20 = (value / 100) * 20;
            Newprice = value + value20;
            Console.WriteLine("Так как книга вышла пять лет назад, цена книги теперь сос" +
                $"тавляет{Newprice}");
        }
    }
}